                ...
                <tr key={s.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                       <button disabled={isLocked} onClick={() => handleStatusChange(s.id, 'present')} className={`p-2 rounded-lg flex items-center gap-1 text-sm font-medium transition-colors ${status === 'present' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-slate-800 text-slate-400'} ${isLocked ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-700'}`}><UserCheck size={16}/> Present</button>
                       <button disabled={isLocked} onClick={() => handleStatusChange(s.id, 'half-day')} className={`p-2 rounded-lg flex items-center gap-1 text-sm font-medium transition-colors ${status === 'half-day' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 'bg-slate-800 text-slate-400'} ${isLocked ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-700'}`}><UserMinus size={16}/> Half Day</button>
                       <button disabled={isLocked} onClick={() => handleStatusChange(s.id, 'absent')} className={`p-2 rounded-lg flex items-center gap-1 text-sm font-medium transition-colors ${status === 'absent' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-slate-800 text-slate-400'} ${isLocked ? 'opacity-50 cursor-not-allowed' : 'hover:bg-slate-700'}`}><UserX size={16}/> Absent</button>
                    </div>
                  </td>
                </tr>
              )
